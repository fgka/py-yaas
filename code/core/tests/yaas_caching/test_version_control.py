...
def test_compare_ok_with_conflict():
    """different commands, always conflicting. 1  2  3  4  5.
    ...
    for ts in (1, 4):
        ...
        assert result.only_in_a.timestamp_to_request.get(ts) == snapshot_a.timestamp_to_request.get(ts)
    # Then: only B
    assert len(result.only_in_b.timestamp_to_request) == 2
    for ts in (3, 5):
        assert result.only_in_b.timestamp_to_request.get(ts) is not None
        assert result.only_in_b.timestamp_to_request.get(ts) == snapshot_b.timestamp_to_request.get(ts)
    # Then: overlapping
    assert result.overlapping is not None
