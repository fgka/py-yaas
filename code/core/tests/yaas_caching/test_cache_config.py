...
_TEST_CACHE_LOCAL_JSON: cache_config.LocalJsonLineCacheConfig = cache_config.LocalJsonLineCacheConfig(
...
class TestCacheConfig:
    ...
    @pytest.mark.parametrize("type_arg", ["", cache_config.CacheType.LOCAL_JSON_LINE.value + "_NOT"])
    ...
    def test_from_json_nok_non_existent_type(self, value: cache_config.CacheConfig):
        with pytest.raises(ValueError):
            cache_config.CacheConfig.from_json(value.as_json().replace(value.type, value.type + "_NOT"))
