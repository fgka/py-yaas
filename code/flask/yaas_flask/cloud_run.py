...
def read_configuration() -> config.Config:
    """
    Based on the environment variables below, reads the GCS stored configuration:
...
async def _kwargs_from_flask_request(
    ...
    """
    Will call the argument with the current instance of :py:class:`flask.Request`.
    It is expected that the output is :py:class:`dict` corresponding to the ``kwargs`` needed
        by the corresponding later call.
    This function is provided so that all errors are properly handled.
...
async def _call_and_respond(
    *,
    ...
    """
    This function will call your function in ``async_fn`` that is responding to a REST API call.
    It is here so that all error handling is done properly.
