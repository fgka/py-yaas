...
class TestFileBasedLockContextManager:
    ...
    def test_ctor_ok_timeout_non_int(self):
        # Given/When
        obj = base.FileBasedLockContextManager(lock_file=self.instance.lock_file, lock_timeout_in_sec=None)
        # Then
    ...
    def test_ctor_nok(self, lock_file: pathlib.Path, lock_timeout_in_sec: int):
        with pytest.raises(Exception):
            base.FileBasedLockContextManager(lock_file=lock_file, lock_timeout_in_sec=lock_timeout_in_sec)
...
def _assert_ts_in_range(value: int, min_value: datetime) -> None:
    ...
    assert int(min_value.timestamp()) <= value, f"Value: {value}, min: {min_value} = {min_value.timestamp()}"
    assert value < int(max_value.timestamp()), f"Value: {value}, max: {max_value} = {max_value.timestamp()}"


def test__default_end_ts_utc_ok():
    # Given
    min_result = datetime.utcnow() + timedelta(days=base._DEFAULT_END_TS_FROM_NOW_IN_DAYS)
    # When
...
def test__default_max_end_ts_utc_ok():
    # Given
    min_result = datetime.utcnow() + timedelta(days=base._MAXIMUM_END_TS_FROM_NOW_IN_DAYS)
    # When
...
class TestStoreContextManager:  # pylint: disable=too-many-public-methods
    ...
    @staticmethod
    def _assert_called_kwargs(called_kwargs: Dict[str, Any], exp_kwargs: Dict[str, Any]) -> None:
    ...
    async def test_write_nok_raises(self, overwrite: bool):
        ...
        # Then: remove
        assert (self.object.called.get(base.StoreContextManager.remove.__name__) is not None) == overwrite
        assert not self.object.has_changed
    ...
    def test__end_ts_utc_from_days_delta_and_logging_ok(self, value: int):
        ...
        # When
        result = base.StoreContextManager._end_ts_utc_from_days_delta_and_logging(value, "test", now=now)
        # Then
    ...
    def test__end_ts_utc_from_days_delta_and_logging_nok(self, value: int):
        now = _TEST_NOW_DT.timestamp()
        with pytest.raises(ValueError):
            base.StoreContextManager._end_ts_utc_from_days_delta_and_logging(value, "test", now=now)
    ...
    async def test_clean_up_ok(self, monkeypatch):
        ...
        assert called_end_ts[0].get("value") == configuration.expired_entries_max_retention_before_archive_in_days
        assert called_end_ts[1].get("value") == configuration.max_retention_archive_before_removal_in_days
        # Then: archive
        called_archive = self.object.called.get(base.StoreContextManager.archive.__name__)
...
class _MyReadOnlyStoreContextManager(base.ReadOnlyStoreContextManager):
    def __init__(self, read_result: event.EventSnapshot = _TEST_EVENT_SNAPSHOT_EMPTY, **kwargs):
...
class TestReadOnlyStoreContextManager:
    ...
    async def test_write_ok(self, overwrite: bool):
        ...
        with pytest.raises(base.StoreError):
            async with self.object:
                await self.object.write(_TEST_EVENT_SNAPSHOT_WITH_REQUEST, overwrite_within_range=overwrite)
        self._assert_context_called()
        assert not self.object.has_changed
