...
def validate_topic_id(value: str) -> None:
    ...
    """
    resource_name.validate_resource_name(value=value, tokens=_PUBSUB_NAME_TOKENS, raise_if_invalid=True)
...
def parse_pubsub(
    *,
    ...
    """This will parse either Cloud Run (wrapped in a
    :py:class:`flask.Request`) or Cloud Function (wrapped in a py:class:`dict`)
    Pub/Sub messages.
    ...
    if isinstance(message, dict):
        ...
        timestamp = _extract_timestamp_from_iso_str(message.get(_MESSAGE_PUBLISH_TIME_KEY))
    else:
...
def _client() -> pubsub_v1.PublisherClient:
    flow_control = types.PublishFlowControl(limit_exceeded_behavior=types.LimitExceededBehavior.BLOCK)
    return pubsub_v1.PublisherClient(publisher_options=types.PublisherOptions(flow_control=flow_control))
