...
_CLOUD_RUN_COMMAND_REGEX: re.Pattern = re.compile(pattern=r"^\s*([^\s\.]+)\.?\s+\.?\s*([\d]+)\s*$", flags=re.IGNORECASE)
"""
Example input::
    min_instances 10
...
@attrs.define(**const.ATTRS_DEFAULTS)
class CloudRunScalingDefinition(scaling.ScalingDefinition):
    """DTO to Hold a Cloud Run scaling definition."""

    def _is_resource_valid(self, value: str) -> bool:
        lst_errors = cloud_run.validate_cloud_run_resource_name(value, raise_if_invalid=True)
        return not lst_errors

    @classmethod
    def from_request(cls, value: request.ScaleRequest) -> "CloudRunScalingDefinition":
        _, res_name = resource_name_parser.canonical_resource_type_and_name(value.resource)
...
class CloudRunScaler(base.ScalerPathBased):
    ...
    @classmethod
    def from_request(cls, *value: Tuple[request.ScaleRequest], **kwargs) -> "CloudRunScaler":
        return CloudRunScaler(*[CloudRunScalingDefinition.from_request(val) for val in value])
