...
async def test_dispatch_ok(monkeypatch):
    ...
    # When
    await pubsub_dispatcher.dispatch(topic_to_pubsub, req, raise_if_invalid_request=True)
    # Then
...
def test__dto_from_event_ok(monkeypatch, value: dto_defaults.HasFromDict):
    ...
    # When
    result = pubsub_dispatcher._dto_from_event(event=value.as_dict(), result_type=value.__class__)
    # Then
...
def test_from_event_ok(monkeypatch):
    ...
    # When
    result = pubsub_dispatcher.from_event(event=request.ScaleRequestCollection.from_lst([req]).as_dict())
    # Then
