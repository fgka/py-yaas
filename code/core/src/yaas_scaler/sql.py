...
@attrs.define(**const.ATTRS_DEFAULTS)
class CloudSqlScalingDefinition(scaling.ScalingDefinition):
    """DTO to Hold a Cloud SQL scaling definition."""

    def _is_resource_valid(self, value: str) -> bool:
        lst_errors = cloud_sql.validate_cloud_sql_resource_name(value, raise_if_invalid=True)
...
class CloudSqlScaler(base.ScalerPathBased):
    ...
    @classmethod
    def from_request(cls, *value: Tuple[request.ScaleRequest], **kwargs) -> "CloudSqlScaler":
        return CloudSqlScaler(*[CloudSqlScalingDefinition.from_request(val) for val in value])
