...
def _create_cloud_sql_scaling_definition(
    *,
    ...
    return sql.CloudSqlScalingDefinition(resource=resource, command=command, timestamp_utc=timestamp_utc)


_TEST_CLOUD_SQL_COMMAND_STR: str = f"{sql.CloudSqlCommandType.INSTANCE_TYPE.value} {_TEST_CLOUD_SQL_INSTANCE_TYPE}"
...
class TestCloudRunScalingDefinition:
    ...
    def test_ctor_nok_wrong_resource(self):
        ...
        with pytest.raises(ValueError):
            sql.CloudSqlScalingDefinition(resource=resource, command=command, timestamp_utc=321)
...
class TestCloudRunScaler:
    ...
    def test__get_enact_path_value_ok(self, field: str, expected: str):
        # Given/When
        result = sql.CloudSqlScaler._get_enact_path_value(resource="resource", field=field, target="target")
        # Then
    ...
    async def test_enact_ok(self, monkeypatch):
        ...
        async def mocked_update_instance(*, name: str, path_value_lst: List[Tuple[str, Optional[Any]]]) -> Any:
