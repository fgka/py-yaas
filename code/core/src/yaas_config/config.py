...
class CacheConfig(dto_defaults.HasFromJsonString):
    ...
    def from_json(cls, json_string: str, context: Optional[str] = None) -> Any:
        ...
        try:
            ...
            raise ValueError(f"Could not parse content into {cls.__name__}. Content: {json_string}") from err
        return result
...
@attrs.define(**const.ATTRS_DEFAULTS)
class LocalJsonLineCacheConfig(CacheConfig):
    ...
    **NOTE**: use for test only.
    ...
    archive_json_line_file: str = attrs.field(validator=attrs.validators.instance_of(str))
...
@attrs.define(**const.ATTRS_DEFAULTS)
class LocalSqliteCacheConfig(CacheConfig):
...
@attrs.define(**const.ATTRS_DEFAULTS)
class GcsCacheConfig(CacheConfig):
...
@attrs.define(**const.ATTRS_DEFAULTS)
class DataRetentionConfig(dto_defaults.HasFromJsonString):
    ...
    expired_entries_max_retention_before_archive_in_days: int = attrs.field(
        ...
        validator=attrs.validators.and_(
            ...
            attrs.validators.ge(MINIMUM_EXPIRED_ENTRIES_MAX_RETENTION_BEFORE_ARCHIVE_IN_DAYS),
        ),
    )
...
@attrs.define(**const.ATTRS_DEFAULTS)
class Config(dto_defaults.HasFromJsonString):
    """Configuration to say to which PubSub topic each YAAS topic goes."""

    calendar_config: CalendarCacheConfig = attrs.field(validator=attrs.validators.instance_of(CalendarCacheConfig))
    cache_config: CacheConfig = attrs.field(validator=attrs.validators.instance_of(CacheConfig))
    ...
    retention_config: DataRetentionConfig = attrs.field(
        default=None,
        converter=attrs.converters.default_if_none(default=attrs.Factory(DataRetentionConfig)),
