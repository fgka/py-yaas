...
class ScalingCommand(dto_defaults.HasFromJsonString):
    ...
    def _target_type() -> Type[Any]:
        ...
        The default, :py:obj:`None` means any type.
        """
        return None
    ...
    def from_command_str(cls, value: str) -> "ScalingCommand":
        """Parse the command :py:cls:`str` into an instance of
        :py:cls:`ScalingCommand`.
...
@attrs.define(**const.ATTRS_DEFAULTS)
class ScalingDefinition(dto_defaults.HasFromJsonString):
    """Defines a DTO to hold the scaling definition."""

    resource: str = attrs.field(validator=attrs.validators.instance_of(str))
    command: ScalingCommand = attrs.field(validator=attrs.validators.instance_of(ScalingCommand))
    timestamp_utc: int = attrs.field(
        default=None,
        validator=attrs.validators.optional(
            attrs.validators.and_(attrs.validators.instance_of(int), attrs.validators.gt(0))
        ),
    )
