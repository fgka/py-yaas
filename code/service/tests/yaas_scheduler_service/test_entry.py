...
from tests import command_test_data, common
...
def _mock_entry(  # pylint: disable=too-complex
    monkeypatch,
    *,
    ...
    async def mocked_update_secret_credentials(  # pylint: disable=unused-argument
        *,
        calendar_id: str,
        secret_name: str,
        initial_credentials_json: Optional[pathlib.Path] = None,  # noqa: F841
    ) -> None:
        nonlocal called
    ...
    def mocked_from_event(  # pylint: disable=unused-argument
        *,
        f_event: Union[flask.Request, Dict[str, Any]],  # noqa: F841
    ...
    monkeypatch.setattr(entry, entry._calendar_snapshot.__name__, mocked_calendar_snapshot)
    monkeypatch.setattr(entry, entry._cache_store_and_snapshot.__name__, mocked_cache_store_and_snapshot)
    monkeypatch.setattr(
...
async def _verify_calendar_credentials_called(called: Dict[str, Any]) -> None:
    ...
    assert called_update.get("calendar_id") == common.TEST_CONFIG_LOCAL_JSON.calendar_config.calendar_id
    assert called_update.get("secret_name") == common.TEST_CONFIG_LOCAL_JSON.calendar_config.secret_name
...
async def _verify_update_cal_cache_called(
    ...
    # Then: calendar
    _verify_calendar_snapshot_called(called.get(entry._calendar_snapshot.__name__), kwargs)
    # Then: cache
    ...
    assert bool(called_cache_snapshot.get("cache_store").called.get(base.StoreContextManager.write.__name__)) == bool(
        expected_cache_store is not None
    )
    assert called_cache_snapshot.get("cache_store").called.get(base.StoreContextManager.clean_up.__name__)
    if expected_cache_store is not None:
        ...
        assert store_called.get(base.StoreContextManager.write.__name__).get("value") == expected_cache_store
        assert (
...
def _verify_cache_snapshot_called(cache_snapshot: Dict[str, Any], kwargs: Dict[str, Any]) -> None:
...
async def test_update_cache_ok(monkeypatch):
    ...
    cache_store = common.MyStoreContextManager()
    called = _mock_entry(monkeypatch, calendar_snapshot=expected, cache_store=cache_store)
    kwargs = dict(
        start_ts_utc=_TEST_START_TS_UTC,
        end_ts_utc=_TEST_END_TS_UTC,
        configuration=common.TEST_CONFIG_LOCAL_JSON,
