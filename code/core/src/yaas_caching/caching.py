...
from yaas_caching import base, event, version_control
...
def update_event_cache(  # pylint: disable=too-complex
    *,
    start_utc: int,
    end_utc: int,
    ...
    * Apply a merging strategy, if necessary, for the overlapping and conflicting events:
    ...
    try:
        ...
        raise RuntimeError(f"Could not read calendar for the range [{start_utc}, {end_utc}]. Got: {err}") from err
    # read cache
    try:
        cached_snapshot = cache_reader(start_utc, end_utc)
    except Exception as err:
        raise RuntimeError(f"Could not read cache for the range [{start_utc}, {end_utc}]. Got: {err}") from err
    # merge
    try:
...
async def update_cache(
    *,
    ...
    Raises:
        TypeError: for input validation
...
async def _update_cache(
    *,
    ...
    try:
        ...
        raise CachingError(
            f"Could not read source starting on {start} for {time_span_in_days} days. Got: {err}"
        ) from err
    # read cache
    try:
        ...
        raise CachingError(
            f"Could not read cache starting on {start} for {time_span_in_days} days. Got: {err}"
        ) from err
    # merge
    try:
