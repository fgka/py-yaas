...
_TEST_INSTANCE_RESOURCE_NAME: str = f"{_TEST_PROJECT}:my-location-123:{_TEST_INSTANCE_NAME}"
...
class _StubSqlInstancesResource:
    ...
    def get(self, project: str, instance: str) -> _StubCloudSqlRequest:  # pylint: disable=unused-argument
        result = _StubCloudSqlRequest(value=self._instance, raise_on_execute=self._raise_on_get_execute)
        self.called[_StubSqlInstancesResource.get.__name__] = locals()
        return result

    def patch(  # pylint: disable=unused-argument
        self, project: str, instance: str, body: Dict[str, Any]  # noqa: F841
    ) -> _StubCloudSqlRequest:
        result = _StubCloudSqlRequest(value=self._patch_result, raise_on_execute=self._raise_on_patch_execute)
...
async def test_can_be_deployed_ok(monkeypatch, status: str, raise_on_get_execute: bool, expected: bool):
    # Given
    instance = xpath.create_dict_based_on_path(cloud_sql_const.CLOUD_SQL_STATE_KEY, status)
    client = _StubSqlInstancesResource(instance=instance, raise_on_get_execute=raise_on_get_execute)
    monkeypatch.setattr(cloud_sql, cloud_sql._sql_instances.__name__, lambda: client)
    # When
...
async def test_update_instance_ok(monkeypatch):
    ...
    instance = xpath.create_dict_based_on_path("status", cloud_sql_const.CLOUD_SQL_STATUS_OK)
    client = _StubSqlInstancesResource(instance=instance, patch_result={})
    monkeypatch.setattr(cloud_sql, cloud_sql._sql_instances.__name__, lambda: client)
    # When
    result = await cloud_sql.update_instance(name=_TEST_INSTANCE_RESOURCE_NAME, path_value_lst=path_value_lst)
    # Then: result
