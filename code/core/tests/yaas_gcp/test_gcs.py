...
def test__validate_and_clean_bucket_and_path_nok_wrong_type(bucket_name: str, path: str):
...
def test__validate_and_clean_bucket_and_path_nok_wrong_value(bucket_name: str, path: str):
...
def test__validate_and_clean_bucket_and_path_ok(bucket_name: str, path: str):
    # Given/When
    res_bucket_name, res_path = gcs.validate_and_clean_bucket_and_path(bucket_name, path)
    # Then
...
def test_list_objects_ok(monkeypatch):
    ...
    # When
    result = list(gcs.list_objects(bucket_name=bucket_name, prefix=prefix, project=project))
    # Then
...
def test_read_object_ok(monkeypatch, expected: bytes):
    ...
    def mocked_bucket(bucket_name: str, project: Optional[str] = None) -> _MyBucket:  # pylint: disable=unused-argument
        nonlocal called
    ...
    assert bool(blob.called.get(_MyBlob.download_as_bytes.__name__)) == bool(expected is not None)
