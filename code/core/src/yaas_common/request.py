...
class Range(dto_defaults.HasFromJsonString):
...
@attrs.define(**const.ATTRS_DEFAULTS)
class ScaleRequest(dto_defaults.HasFromJsonString):
...
def _convert_list_dict(value: List[Union[ScaleRequest, Dict[str, Any]]]) -> List[ScaleRequest]:
...
@attrs.define(**const.ATTRS_DEFAULTS)
class ScaleRequestCollection(dto_defaults.HasFromJsonString):
    """Intended to be used to aggregate multiple :py:class:`ScaleRequest`
    through PubSub.

    Instead of instantiating it directly, use the factory method to
    clean it up.
    """

    collection: List[ScaleRequest] = attrs.field(
        converter=_convert_list_dict,
        validator=attrs.validators.deep_iterable(member_validator=attrs.validators.instance_of(ScaleRequest)),
    )
