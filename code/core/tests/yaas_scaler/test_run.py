...
def _create_cloud_run_scaling_definition(
    *,
    ...
    return run.CloudRunScalingDefinition(resource=resource, command=command, timestamp_utc=timestamp_utc)
...
class TestCloudRunScalingDefinition:
    ...
    def test_ctor_nok_wrong_resource(self):
        ...
        command = run.CloudRunScalingCommand(parameter=run.CloudRunCommandType.MIN_INSTANCES.value, target=123)
        # When/Then
        with pytest.raises(ValueError):
            run.CloudRunScalingDefinition(resource=resource, command=command, timestamp_utc=321)
...
class TestCloudRunScaler:
    ...
    def test__get_enact_path_value_ok(self, field: str, expected: str):
        # Given/When
        result = run.CloudRunScaler._get_enact_path_value(resource="resource", field=field, target="target")
        # Then
    ...
    async def test_enact_ok(self, monkeypatch):
        ...
        async def mocked_update_service(*, name: str, path_value_lst: List[Tuple[str, Optional[Any]]]) -> Any:
