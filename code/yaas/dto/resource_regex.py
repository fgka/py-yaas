...
class ResourceRegex(  # pylint: disable=too-few-public-methods
    ...
    prj_loc_name_order: Tuple[int] = attrs.field(
        validator=attrs.validators.instance_of(tuple)
    )

    @prj_loc_name_order.validator
    def _validate_prj_loc_name_order(
        self, attribute: attrs.Attribute, value: Any
    ) -> None:
        if not isinstance(value, tuple):
            raise TypeError(
                f"Attribute {attribute.name} must be a {tuple.__name__}. "
    ...
    def prj_loc_name(self, value: str) -> Optional[Tuple[str, str, str]]:
        ...
        if not isinstance(value, str):
            raise TypeError(
                f"Argument must be a {str.__name__} instance. "
                f"Got: <{value}>({type(value)})"
            )
        # logic
        result = None
...
@attrs.define(**const.ATTRS_DEFAULTS)
class ResourceRegexParser(  # pylint: disable=too-few-public-methods
    dto_defaults.HasFromJsonString
):
    """
    ...
    regex_lst: List[ResourceRegex] = attrs.field(
        validator=attrs.validators.deep_iterable(
            member_validator=attrs.validators.instance_of(ResourceRegex)
        ),
    )
    canonical_form_tmpl: str = attrs.field(validator=attrs.validators.instance_of(str))
    type: ResourceType = attrs.field(
        validator=attrs.validators.instance_of(ResourceType)
    )
    ...
    def canonical(self, value: str) -> Optional[str]:
        """
        If the value is valid, will return the canonical string representing the resource.
