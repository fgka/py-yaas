...
def _default_end_ts_utc() -> int:
    ...
    :py:data:`_DEFAULT_END_TS_FROM_NOW_IN_DAYS`."""
...
def _default_max_end_ts_utc() -> int:
    """Returns current UTC timestamp +
    :py:data:`_MAXIMUM_END_TS_FROM_NOW_IN_DAYS`."""
...
class FileBasedLockContextManager(contextlib.AbstractAsyncContextManager):
    ...
    def lock_timeout_in_sec(self) -> int:
        """How long to wait for a lock before raising
        :py:class:`StoreLockTimeoutError`."""
    ...
    def _lock(self) -> bool:
        """Blocks all file operations globally, so that writes don't trump each other and corrupt the files.
    ...
    async def __aexit__(
        self,
        __exc_type: Optional[Type[BaseException]] = None,  # noqa: F841
        __exc_value: Optional[BaseException] = None,  # noqa: F841
        __traceback: Optional[types.TracebackType] = None,  # noqa: F841
    ) -> Optional[bool]:
        return self._unlock()


class StoreContextManager(contextlib.AbstractAsyncContextManager, abc.ABC):
    """Generic class to define a :py:class:`event.EventSnapshot` store as an `Asynchronous Context Manager`_.
    ...
    def _effective_start_ts_utc(self, value: Optional[Union[int, float, datetime]] = None) -> Optional[Any]:
    ...
    def _effective_end_ts_utc(self, value: Optional[Union[int, float, datetime]] = None) -> Optional[int]:
        result = self._get_int_ts(value)
        if result is None:
    ...
    async def _open(self) -> None:
        """To be overwritten in case of resource allocation, like opening files or database connections."""

    async def _close(self) -> None:
        """To be overwritten in case of resource disposal, like closing files or database connections."""
    ...
    async def read(
        ...
        *,
        ...
        """Will retrieve a snapshot from the store that contains all events within the range specified by
        ...
        try:
            result = await self._read(start_ts_utc=start_ts_utc, end_ts_utc=end_ts_utc, is_archive=is_archive)
    ...
    async def write(
        ...
        *,
        ...
        """Will store the snapshot and, if overwrite is set to :py:obj:`True`, will remove all entries within the period
    ...
    async def remove(
        ...
        *,
        ...
        """Will remove all events within the range specified by ``[start_ts_utc, end_ts_utc]`` and return them.
    ...
    async def archive(
        ...
        *,
        ...
        """Similar to :py:meth:`remove` but will move the data out of the current store into a _cold_ storage. The
        archiving content is the one within the range specified by ``[start_ts_utc, end_ts_utc]`` and return them.
    ...
    async def clean_up(
        ...
        removed = await self.remove(start_ts_utc=1, end_ts_utc=remove_end_ts_utc, is_archive=True)
        return archived, removed

    @staticmethod
    def _end_ts_utc_from_days_delta_and_logging(value: int, log_msg_prefix: str, *, now: Optional[int] = None) -> int:
...
class ReadOnlyStoreContextManager(StoreContextManager, abc.ABC):
    ...
    async def _write(
        ...
        raise StoreError(f"This is a {self.__class__.__name__} instance which is also read-only.")
